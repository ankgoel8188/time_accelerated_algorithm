%% P20.11: Discrete Finite Time Estimator (theta0) Plots
% Author:   Turibius Rozario
% Advisor:  Dr. Ankit Goel
% Date:     2023-03-27
% Ensure to call P2011...py first! This script simply plots the results
% from the python script to compare SGD, Adam, and FTE optimizers as the
% theta initialization changes

clear; clc; close all

% Parameters
FILE_LOC = "Pytorch\P2011\";
label_NI = ["1", "10", "10"];
label_NO = ["1", "1", "5"];
...
% Reading the files
all_costs = cell(3, 1);
full_costs = zeros(epochs(end), tests);
for ii=1:3
    all_costs{ii} = cell(3, 1);
    curr_filename = FILE_LOC + "costs_" + test_types(ii) + "_";
    for ij=1:3
        for jj=0:tests-1
            full_costs(:, jj+1) = readmatrix(curr_filename + ...
                string(ij) + string(jj) + ".csv");
        end
        all_costs{ii}{ij} = mean(full_costs, 2);
    end
end
...
% exportgraphics(f, "FiguresLaTeX\P2011_OptimizerComparisonWithFTE.png")